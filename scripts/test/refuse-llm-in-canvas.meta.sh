...
# Phase 1 (positive): expect ZERO matches on clean tree
...
# Phase 2 (negative): plant a probe; expect grep to find it
trap 'rm -f "$TEMP_FILE"' EXIT
printf '// LLM prompt( -- Phase 17 meta-test probe\n' > "$TEMP_FILE"
if ! grep_canvas > /dev/null 2>&1; then
	echo "META FAIL -- phase 2 negative: probe planted but grep returned no match"
	exit 1
fi
