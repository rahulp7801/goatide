...
// write-RPC method names (atomicAccept / proposeEdit / recordRejection /
// recordContractOverride) so the read-only contract cannot drift.
//
 ...
 *
 * Banned methods (DEEP-05 Mandate B + DEEP-04 Mandate D): `atomicAccept`, `proposeEdit`,
 * `recordRejection`, `recordContractOverride`. The CI gate `refuse-deep05-write.sh`
 * grep-fences `inspector/` against any TS file containing those identifiers — even in a
 * comment, even as a string literal — so this type is the *contract*, the gate is the
 * *enforcement*.
 *
