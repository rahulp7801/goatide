...
async function main() {
	...
	try {
		...
		// === SC13-4 kernel-health gate (Phase 13 Plan 13-04 — live) ===
		//
		// Assertions:
		//   (a) No "NODE_MODULE_VERSION mismatch" substring in the renderer log (CLOSE-01).
		//   (b) No "kernel-degraded" banner present in the workbench DOM (CLOSE-01/CLOSE-02).
		...
		//
		// Counts as one compound assertionsPassed increment (assertionsPassed total: 5).

		// (a) No ABI mismatch in renderer console log.
		// Access via the `contents` variable already in scope from SC10-5.
		...
		assertionsPassed++; // SC13-4 compound assertion — counts as 1 (assertionsPassed now targets 5)
	} finally {
	...
	if (assertionsPassed !== 5) {
		throw new Error('SC#5 fail: only ' + assertionsPassed + '/5 assertions passed before exit');
	}

	console.log('[freshclone-smoke-cdp] SC#5: all 5 assertions PASS (SC13-4 kernel-health gate live)');
}
