...
### Distribution (Phase 22 — external-cert deferred 2026-05-19)

- [~] **C1**: macOS notarization via `@electron/notarize ^3.1.1` (notarytool only; altool removed by Apple). `beforeSign` hook re-signs `better_sqlite3.node` + all `.node` files with hardened runtime before main `.app` signing. `xcrun stapler staple` post-notarize embeds ticket in DMG. Requires Apple Developer account ($99/yr — operational prereq). **Infrastructure complete (Plan 22-02, commit `0763ed9fe6c`) + CI workflow scaffolded (`.github/workflows/release-mac.yml`, commit `8e98a3c749f`). Deferred 2026-05-19 — operator opted out of commercial cert procurement this milestone. v2.1 ships unsigned macOS builds; recipients use right-click → Open to bypass Gatekeeper. Re-open when an Apple Developer account is procured.**
- [~] **C2**: Windows code-signing via **Azure Trusted Signing** (CI-friendly path -- EV USB tokens cannot be used in GitHub Actions). `win.azureSignOptions` in `electron-builder.yml`. SmartScreen reputation accumulates over time (no instant EV bypass post-March 2024). Requires Azure Trusted Signing account (operational prereq). **Infrastructure complete (Plan 22-03, commits `8f095bd2991`..`f915f395c69`) + CI workflow scaffolded (`.github/workflows/release-win.yml`, commit `8e98a3c749f`). Deferred 2026-05-19 — paired with C1 deferral (symmetric "no commercial cert procurement" stance for v2.1). v2.1 ships unsigned Windows installers; recipients click SmartScreen "More info" → "Run anyway". Re-open when an Azure Trusted Signing account is procured.**
...
| C1 | 22 — Distribution | Deferred 2026-05-19 (operator opted out of Apple Developer account procurement this milestone; infrastructure + CI workflow ready — Plan 22-02 commits `3cf21910b6c`..`0763ed9fe6c` + `.github/workflows/release-mac.yml` commit `8e98a3c749f`; re-open when an Apple Developer account is procured) |
| C2 | 22 — Distribution | Deferred 2026-05-19 (paired with C1 deferral — no commercial cert procurement this milestone; infrastructure + CI workflow ready — Plan 22-03 commits `8f095bd2991`..`f915f395c69` + `.github/workflows/release-win.yml` commit `8e98a3c749f`; re-open when an Azure Trusted Signing account is procured) |
