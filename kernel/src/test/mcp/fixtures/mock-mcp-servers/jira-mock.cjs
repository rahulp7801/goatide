...
// kernel/src/test/mcp/fixtures/mock-mcp-servers/jira-mock.cjs — Phase 6 stdio MCP server
// fixture for Jira. Tools: ticket_read({key}) -> deterministic Jira issue payload.
//
// CLI args:
	...
	if (req.method === 'initialize') {
		send({
			...
			result: {
				...
				serverInfo: { name: 'jira-mock', version: '0.0.1' },
				capabilities: { tools: {} },
			},
		});
		if (mode === 'crash') {
	...
	if (req.method === 'tools/call') {
		if (mode === 'revoked') {
			send({
				...
				id: req.id,
				result: {
					content: [{ type: 'text', text: JSON.stringify({ errorMessages: ['Login required'], errors: {} }) }],
					isError: true,
				},
			});
			return;
			...
			send({
				...
				result: {
					content: [{ type: 'text', text: JSON.stringify({
						summary: 'BigDecimal arithmetic in checkout calculator',
						description: 'Discount amount must use BigDecimal to avoid float drift in subtotal totals. Contract.',
						assignee: 'qa-02',
						status: 'In Review',
						priority: 'Critical',
						fix_versions: ['v1.0.0'],
